...
class DLRM_Net(nn.Module):
    ...
    # dump all embedding table
    def dump_emb(self, savepath, idx):
        emb_params = self.emb_l.state_dict()
        print(self.emb_l)
    ...
    def dump_data(self,data, type, savepath, filename):
        if type=="binary":
            finalpath = savepath+"/embedding_outputs/"+filename
            data.tofile(finalpath)
        elif type=="numpy":
            finalpath = savepath+"/embedding_outputs/"+filename
            np.save(finalpath,data)
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            # print("sparse_index_group ", sparse_index_group_batch, sparse_index_group_batch.shape)
            # print("sparse_offset_group ", sparse_offset_group_batch, sparse_offset_group_batch.shape)
            # embedding lookup
    ...
    def distributed_forward(self, dense_x, iter, epoch, is_test, lS_o, lS_i):
        ...
        if  args.enable_compress and not is_test:
            ...
            compressed_data, compression_ratio = my_emb_comp.compress(compressor=args.compressor,format="table_wise_seperate",data=ly_data,tolerance=r_tolerance)
            new_ly = my_emb_comp.decompress(args.compressor, "table_wise_seperate", compressed_data, data_shape, data_type)
            if iter == 0:
            ...
            if iter % 1024 == 0 and ext_dist.rank==0:
                ...
                for i in range(len(ly_data)):
                    ...
                    print("L2 norm: ", np.linalg.norm(tmp_delta))
                '''
        ...
        # TODO
        # Figure out what 'ly' is right here.
        ly = a2a_req.wait()
        ly = list(ly)
    ...
    def parallel_forward(self, dense_x, iter, epoch, is_test, lS_o, lS_i):
        ...
        if self.parallel_model_is_not_prepared:
            ...
            #for k, emb in enumerate(self.emb_l):
            #    print(emb,next(emb.parameters()).device)
        ...
        '''
        It is not the right place to apply embedding vector compression in real workflow
        If we want to apply this, we need to change
        '''
        # embeddings
        ly = self.apply_emb(lS_o, lS_i, self.emb_l, self.v_W_l)
        ly_devices = []
        ...
        if  args.enable_compress and not is_test:
            ...
            data_shape = ly_data[0].shape
            r_tolerance = 1
            compressed_data, compression_ratio = my_emb_comp.compress(compressor="ZFP_compressor",format="table_wise_seperate",data=ly_data,tolerance=r_tolerance)

            new_ly = my_emb_comp.decompress("ZFP_compressor", "table_wise_seperate", compressed_data, data_shape, data_type)
            if iter == 0:
                print("Compression method: ZFP, Parameters: ", r_tolerance)
            for i in range(len(new_ly)):
                ly[i].data = torch.from_numpy(new_ly[i]).data.to(ly_devices[i])
        ...
        # debug prints
        # print("The size of embedding table is ",len(ly))
        test_scatter_list = ["dfoaisdfjoidf",int(i),12.50,dict(),1,2,3,4]
        test_scatter = scatter(test_scatter_list, device_ids, dim=0)
        print(test_scatter)
        # interactions
        z = []
        ...
        # print(z)
        '''  
        print("######### The Following Interaction of Model #########")
        print("Bottom MLP")
        for k, _ in enumerate(x):
            print(_.device)
        print("Embedding Layers")
        for k, _ in enumerate(ly):
            for i in range(len(_)):
                print(k, len(_[i]),_[i].device)
        '''
...
def run():
    ...
    print("Devices number is ", ndevices)
    ...
    ext_dist.barrier()
    # there is a problem that I do not know worker_name should be 0 or something else for one node and multiple GPU
    if not args.inference_only:
        k = 0
        total_time_begin = 0
        with torch.profiler.profile(
                ...
                schedule=torch.profiler.schedule(
                    ...
                    warmup=1,
                    active=2),
                on_trace_ready=torch.profiler.tensorboard_trace_handler('./Log_DLRMresult', worker_name='batch_2048'),
                record_shapes=True,
                ...
                while k < args.nepochs:
                    ...
                    for j, inputBatch in enumerate(train_ld):
                        ...
                        # each epoch, save the embedding table once
                        if args.save_embedding and j == 0:
                            dlrm.dump_emb(savepath, k)
                        ...
                        '''
                        if args.enable_profiling and j < 5:
                            p.step()
