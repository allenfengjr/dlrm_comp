...
def draw_histogram(data, table, iter):
    ...
    plt.savefig(f"{savepath}/hist_table_{table}_iter_{iter}_quan.png")
    plt.clf()
...
if __name__ == "__main__":
    ...
    print(f"max value is {data.max()}, min value is {data.min()}")
    #draw_histogram(data, table, iter)
    #data after quantization
    data = quantization(data, eb, filepath)
    draw_histogram(data, table, iter)
    sys_data = np.random.normal(0, 0.02, (32,16))
    subset = data[:32*emb_len]  # Select the first 128 rows
    subset = subset.reshape(32,emb_len)
    ...
    draw_heatmap(sys_data, table, iter)
