...
void compress(char *inPath, char *cmpPath, SZ::Config conf) {
    ...
    auto quantizer = SZ::LinearQuantizer<T>(conf.relErrorBound, conf.quantbinCnt / 2);
    auto encoder = SZ::HuffmanEncoder<int>();
    auto lossless =  SZ::Lossless_zstd();
    std::cout<<conf.absErrorBound<<std::endl;
    quantizer.precompress_data();
    for (size_t i=0;i<conf.num;i++){
        //if (i%(1024*16)==0)std::cout<<"pre-quant"<< data[i]<<std::endl;
        quant_inds[i]=quantizer.quantize(data[i], 0);//use zero as pred value, equal to no-prediction
        //if (quant_inds[i]!=0)std::cout<<"after-quant"<< quant_inds[i]<<std::endl;

    }
    // postcompress_data() is a virtual function, what does this do?
    quantizer.postcompress_data();
    encoder.preprocess_encode(quant_inds, 0); // change to 255 from 0, nothing change, this should be stateNum param
    size_t bufferSize = 1.2 * (encoder.size_est() + sizeof(T) * quant_inds.size());

    SZ::uchar *buffer = new SZ::uchar[bufferSize];
    SZ::uchar *buffer_pos = buffer;

    // save function: save metadata(huffman tree etc.) into buffer
    quantizer.save(buffer_pos);
    encoder.save(buffer_pos);
    long int encode_outsize = encoder.encode(quant_inds,buffer_pos);
    printf("compressed size is, %ld\n",encode_outsize);
    encoder.postprocess_encode();
    // encode size replace compressed_size
    printf("buffer diff size is, %ld\n",(buffer_pos - buffer));
    printf("compression ratio, %.2f Without lossless\n", conf.num * 1.0 * sizeof(T) / (buffer_pos - buffer));
    // I think 'buffer_pos - buffer' is number of bytes
    
    //end change
    assert(buffer_pos - buffer < bufferSize);
    size_t compressed_size;
    SZ::uchar *lossless_data = lossless.compress(buffer, buffer_pos - buffer, compressed_size);
    lossless.postcompress_data(buffer);

    printf("CR = %.2f with lossless compression\n", conf.num * 1.0 * sizeof(T) / compressed_size);
