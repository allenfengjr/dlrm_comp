...
class DLRM_Net(nn.Module):
    ...
    def parallel_forward(self, dense_x, iter, epoch, is_test, lS_o, lS_i):
        ...
        if args.enable_compress and not is_test:
            ly_data = [_.detach().cpu().numpy() for _ in ly]
            q_ly = [torch.ops.fbgemm.FloatToHFP8Quantized(_, 4, 1, 10.0) for _ in ly]
            ...
            for i in range(len(dq_ly)):
                ly[i].data = torch.from_numpy(dq_ly[i]).data.to(ly_devices[i])
            
            if iter % 1024 == 0:
                #print("Compression ratio,", compression_ratio)
                for i in range(len(ly_data)):
                    tmp_delta = ly_data[i] - dq_ly[i]
